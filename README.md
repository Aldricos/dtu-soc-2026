...
This repository contains a starting point for integrating a Chisel project into the Caravel harness using Librelane. It includes a Chisel top-level module (`CaravelTop`) containing a simple GCD calculator which is connected to the Caravel's wishbone bus and GPIOs. The verilog for the `CaravelTop` module is generated by running:
